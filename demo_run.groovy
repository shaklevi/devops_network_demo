pipeline {
    ...
    stages {
        ...
        stage('Run Docker Images') {
            steps {
                script {
                    // 1. Define the arguments using standard Groovy string interpolation
                    def dockerArgs = "-u 1000:1000 --group-add ${env.DOCKER_GID} -v /var/run/docker.sock:/var/run/docker.sock"
                    
                    // 2. Run the container using the .inside() method
                    docker.image('docker:latest').inside(dockerArgs) {
                        // All commands inside this block run contextually INSIDE your container
                        sh 'docker ps'
                    }
